...
The GenerateData class provides functionality for generating and processing data 
from oscillatory shear experiments. It supports various data generation scenarios,
including dimensional analysis cases and standard oscillatory shear cases, with 
options for data splitting into training and validation sets.
...
class GenerateData:
    """
    A utility class for generating and processing experimental data from oscillatory shear tests.
    Provides methods for data loading, feature calculation, and dataset splitting for machine learning applications.
    """
    
    def __init__(self):
        """Initialize the GenerateData class with default time arrays."""
        self.t = np.linspace(0, 10, 1000)           # Default time array
        self.t_collection = np.linspace(0, 10, 200000)  # Dense time array for collection
    ...
    def create_DimensionalCase_data(self, name_list, project_root, collection_multiple, 
        ...
        """
        Generates dimensional data for cases using the Dimensionless-Learning approach.
        
        Args:
            name_list (list): List of folder names containing experimental data
            project_root (str): Root directory of the project
            collection_multiple (int): Multiplier for dense time array resolution
            directory_path (str): Path to directory containing data folders
            log_flag (bool): If True, uses log(gamma0) instead of gamma0
        
        Returns:
            tuple: Contains:
                - Data_Total: Complete dataset
                - Data_Train: Training subset
                - Data_Validation: Validation subset
                - Data_Collection: Combined dataset for PINN training
                - lb: Lower bounds of input features
                - ub: Upper bounds of input features
                - t: Time array from experimental data
        """
        Data_Total = np.array([])
        ...
        for gam_value in name_list:
            ...
            Data_Total_i[:, 4] = tau_xx.flatten() / P0
            Data_Total_i[:, 5] = np.log(gamma0) if log_flag else gamma0
            
            # Create dense collection dataset
        ...
        # Create collection dataset (for PINN training with fixed SINDy model)
        Data_Train_modified = np.delete(Data_Train, 4, axis=1)  # Remove sigma column
        ...
        # Prepare measurement data and bounds
    ...
    def create_oscillatoryshear_data(self, name_array, directory, 
        ...
        Args:
            ...
            directory_path (str): Path to directory containing data folders
            file_prefix (str): Prefix for data folder names
            log_flag (bool): If True, uses log(gamma0) instead of gamma0
        ...
        for gam_value in name_array:
            # Format gamma value for filename matching
    ...
    def create_synthetic_data(self, gamma0_values):
        ...
        Returns:
            tuple: Contains deformation and stress arrays
        """
        # Define deformation functions
        def gam(t, gamma0):
            return gamma0 * np.sin(t)

        def gam_t(t, gamma0):
            return gamma0 * np.cos(t)

        def gam_tt(t, gamma0):
            return -gamma0 * np.sin(t)
        
        def sigma(t, gamma0):
            return gamma0 * (np.sin(t) + np.cos(t))
...
if __name__ == "__main__":
    # Example usage (paths should be adjusted based on actual directory structure)
    current_dir = os.getcwd()
    data_dir = r'H:/Batch_OscillatoryShear_2/'
    file_prefix = r'p10000_gam'
    
    # Example gamma values (uncomment to use)
    # gamma_values = np.arange(0.10, 0.12, 0.01)
    
    # Assuming classification_results is defined elsewhere
    # gamma_folders = classification_results[1]['folders']
    
    project_root = os.path.abspath(os.path.join(current_dir, os.pardir))
    data_generator = GenerateData()
    
    # Example initialization (uncomment and adjust parameters as needed)
    # data_total, data_train, data_val, data_collect, lb, ub, t = data_generator.create_oscillatoryshear_data(
    #     gamma_values, current_dir, data_dir, file_prefix, log_flag=False)
