...
%% Define data list
data_list = [0.005, 0.006, 0.007, 0.008, 0.009, 0.010];

%% Automatically load data
data = cell(size(data_list));
base_path = 'H:\Batch_OscillatoryShear_6_2\ds\p10000_';
file_tail = '_rho2500_nH15_T10_Ep5e7_mu0.50_Gamma0.50\post\Tau_Gammat.mat';
% file_tail = '_rho2500_nH15_T10_Ep5e7_mu0.50_I5.00e-04\post\Tau_Gammat.mat';

for i = 1:length(data_list)
    % Construct full file path
    p_value = data_list(i);
    file_path = [base_path, 'ds', num2str(p_value, '%0.3f'), file_tail];
...
for i = 1:num_curves
    if ~isempty(data{i})
        ...
        diameter_value = data_list(i);
        
        % Plot curve and save handle for legend
        h = plot(current_data.t(1:1201,1)-current_data.T, current_data.tau_xx(1:1201,1)./current_data.P0, ...
            'Color', current_color, ...
            'LineWidth', 2.5);
        
        % Save legend entry
        legend_entries{i} = sprintf('d_s = %d mm', diameter_value*1000);
...
% Create colorbar
c = colorbar;
c.Label.String = 'Diameter (mm)';
c.Label.FontSize = 20;
c.Label.FontName = 'Arial';
set(c, 'FontSize', 18, 'FontName', 'Arial');

% Set colorbar range
caxis([min(data_list)*1000, max(data_list)*1000]);
% Set colorbar ticks, displaying 7 ticks
c.Ticks = linspace(min(data_list)*1000, max(data_list)*1000, 7);

% Mark colors corresponding to data points on colorbar
hold on
for i = 1:num_curves
    % Calculate position of current diameter value on colorbar
