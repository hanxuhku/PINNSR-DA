...
% base_path = 'H:\Batch_OscillatoryShear_6_2\nH\p10000_ds0.005_rho2500_';
base_path = 'H:\Batch_OscillatoryShear_6_2\nH_controlV\p10000_ds0.005_rho2500_';
file_tail = '_T10_Ep5e7_mu0.50_Gamma0.50\post\Tau_Gammat.mat';

for i = 1:length(data_list)
    % Construct full file path
    nh_value = data_list(i);
    file_path = [base_path, 'nH', num2str(nh_value, '%d'), file_tail];
...
for i = 1:num_curves
    if ~isempty(data{i})
        ...
        % Store legend entry
        legend_entries{i} = sprintf('nH = %d', nh_value);
...
% Set colorbar ticks, displaying 6 ticks
c.Ticks = linspace(min(data_list), max(data_list), 6);

% Mark positions corresponding to data points on colorbar
hold on
for i = 1:num_curves
    % Calculate normalized position of current nH value on colorbar
