...
%% Define data list (density values)
data_list = [2300, 2400, 2500, 2600, 2700, 2800, 2900];

%% Automatically load data
data = cell(size(data_list));
base_path = 'H:\Batch_OscillatoryShear_6_2\rho\p10000_ds0.005_';
file_tail = '_nH15_T10_Ep5e7_mu0.50_Gamma0.50\post\Tau_Gammat.mat';

for i = 1:length(data_list)
    % Construct complete file path
    rho_value = data_list(i);
    file_path = [base_path, 'rho', num2str(rho_value, '%d'), file_tail];
...
for i = 1:num_curves
    if ~isempty(data{i})
        ...
        % Extract density value for legend
        rho_value = data_list(i);
        ...
        % Store legend entry
        legend_entries{i} = sprintf('ρ = %d kg/m³', rho_value);
